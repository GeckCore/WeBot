module.exports = {
    ...
    execute: async ({ sock, remitente, msg, textoLimpio, getMediaInfo, downloadContentFromMessage, quoted }) => {
        ...
        try {
            ...
            // 4. Eliminación del mensaje original
            // Se usa un bloque try/catch silencioso por si el bot no es admin y no puede borrar mensajes de terceros
            try {
                await sock.sendMessage(remitente, { delete: msg.key });
            } catch (e) {
                // Falla silenciosamente si no tiene permisos
            }
